...
// GenerateClientOrderID generates a unique client order ID using cryptographically secure random
func (mb *MessageBuilder) GenerateClientOrderID() uint64 {
	// Use timestamp for upper 32 bits
	// UnixNano() returns int64 but is always positive, safe to convert
	nanoTime := time.Now().UnixNano()
	if nanoTime < 0 {
		// This should never happen, but handle it defensively
		nanoTime = 0
	}
	// #nosec G115 - nanoTime is guaranteed to be non-negative
	timestamp := uint64(nanoTime) % uint64(1e9)

	// Use crypto/rand for lower 32 bits (secure random)
	randomBytes := make([]byte, 4)
	_, err := rand.Read(randomBytes)
	if err != nil {
		// Fallback to timestamp-based ID if crypto/rand fails
		mb.logger.Warn("Failed to generate secure random bytes, using timestamp fallback", zap.Error(err))
		return timestamp
	}

	randomValue := uint64(binary.BigEndian.Uint32(randomBytes))
	clientOrderID := (timestamp << 32) | randomValue
